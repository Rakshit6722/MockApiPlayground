...
import { createMockRoute } from '../_services/mockApi';
  ...
  const findAllMocks = async () => {
    try {
      ...
      if (response.status === 404) {
        console.log('No mocks found for this user');
        setUserMocks([]);
        setIsLoading(false);
        return;
      }
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch mocks: ${response.status}`);
      }

      const data = await response.json();
      console.log('Fetched mocks:', data);
      setUserMocks(data);
        ...
        >
          ...
          >
            ...
            {isLoading ? (
              ...
              <div className="p-4">
                <EndpointTable userMocks={userMocks} />
              </div>
            )}
          </motion.div>
        </motion.div>
